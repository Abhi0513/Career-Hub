...
import com.cdac.careerhub.entity.IRecruiterJobs;
...
public interface JobPostActivityRepository extends JpaRepository<JobPostActivity, Integer> {
    ...
    @Query(value = """
		SELECT * FROM job_post_activity j INNER JOIN job_location l on j.job_location_id=l.id  WHERE j\
		.job_title LIKE %:job%\
		 AND (l.city LIKE %:location%\
		 OR l.country LIKE %:location%\
		 OR l.state LIKE %:location%) \
		 AND (j.job_type IN(:type)) \
		 AND (j.remote IN(:remote))\s""", nativeQuery = true)
